...
} from '../src/utils/notebook-ai-execution'
import {
  createExampleWorkspace,
  type ExampleRuntime,
} from '../src/utils/example-workspace'

test('notebook AI execution detects runtime and WebContainer file changes', () => {
  const runtime = {
    type: 'webcontainer',
  ...
  assert.equal(
    requiresNotebookWorkbenchReset(null, client, {
      runtime: null,
  ...
  assert.equal(
    requiresNotebookWorkbenchReset(runtime, webContainer, {
      runtime,
...
test('notebook AI hidden files follow the validated workspace', () => {
