...
import type {
  ...
  NotebookAiEvalWorkspaceCheck,
} from './types'

export function gradeNotebookAiWorkspace(
  execution: NotebookAiExecution,
  checks: ReadonlyArray<NotebookAiEvalWorkspaceCheck>,
) {
  return checks.map((check): NotebookAiEvalCheckResult => {
    switch (check.kind) {
      case 'runtime': {
