...
import {
  applicationStarterDeployBodySchema,
...
export const Route = createFileRoute("/api/application-starter/deploy/github")({
  server: {
    handlers: {
      /**
      ...
      POST: async ({ request }: { request: Request }) => {
        const [authIndex, authGithub, githubRepo] = await Promise.all([
            import("~/auth/index.server"),
            import("~/auth/github.server"),
        ...
        try {
          ...
          body = parseApplicationStarterRequest(applicationStarterDeployBodySchema, bodyResult.body);
        } catch {
          return deployErrorResponse(
            {
              success: false,
        ...
        // Compile the project, or use files generated by the client-only applicationStarter.
        ...
        const pushResult = await pushFiles(authState.accessToken, {
          ...
          files: compiledFiles,
          message: "Initial commit from TanStack Application Starter",
        });
