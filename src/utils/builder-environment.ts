...
function describeImport(
  specifier: keyof typeof builderImports,
  description: string,
) {
  return { specifier, description }
}

export const builderImportAliases = [
  ...
  describeImport(
    '@tanstack/markdown/react',
    'TanStack Markdown React renderer using the builder React instance',
  ),
...
export const builderModuleRules = [
...
export const builderEnvironmentRules = [
...
export const builderThemeRules = [
  'The iframe root has either a light or dark class and follows the TanStack site theme after every run.',
  'Use --builder-background and --builder-foreground for theme-aware colors. Use --builder-error for errors.',
]
...
export const builderStarterSource = `import { useState } from 'react'
import { createRoot } from 'react-dom/client'
...
.sandbox__button {
  ...
  border: 1px solid
    color-mix(
      in srgb,
      var(--builder-foreground) 45%,
      var(--builder-background)
    );
  border-radius: 0.625rem;
  background: color-mix(
    in srgb,
    var(--builder-foreground) 8%,
    var(--builder-background)
  );
  color: var(--builder-foreground);
  font: inherit;
...
.sandbox__button:hover {
  background: color-mix(
    in srgb,
    var(--builder-foreground) 14%,
    var(--builder-background)
  );
}

.sandbox__button:focus-visible {
  outline: 2px solid var(--builder-foreground);
  outline-offset: 3px;
}
\`
...
export function generateBuilderLlmsTxt() {
  const lines = [
    ...
    '',
    'Existing `#code=` builder links use this contract:',
    '',
    ...builderModuleRules.map((rule) => `- ${rule}`),
    '',
    '### Starter TSX module',
    '',
    '```tsx',
    ...
    '## Browser environment',
    '',
    ...builderEnvironmentRules.map((rule) => `- ${rule}`),
    '',
    ...
    '## Theme',
    '',
    ...builderThemeRules.map((rule) => `- ${rule}`),
    '',
    '## Sharing protocol',
    '',
    ...
    '',
    '`https://tanstack.com/builder?title=<title>&description=<description>#code=<source>`',
    '',
    'To produce `<source>`, UTF-8 encode the TSX module, gzip the bytes, encode them as base64url, and omit `=` padding.',
    '',
    'Small multi-file projects use `https://tanstack.com/builder#project=<project>`. The decoded JSON and the large-project POST body are exactly `{ "version": 1, "title": string, "description": string, "workspace": <workspace> }`. Encode and decode it with the same UTF-8, gzip, and unpadded base64url process.',
    '',
