...
import { parseBuilderAiExecution } from '../../src/utils/builder-ai'
import { builderAiEvalCases } from './cases'
import { gradeBuilderAiWorkspace } from './grade'
import type {
  BuilderAiEvalAction,
  BuilderAiEvalAttemptResult,
  BuilderAiEvalCase,
  BuilderAiEvalCheckResult,
  BuilderAiEvalPreviewCheck,
  BuilderAiEvalPreviewStep,
} from './types'
...
const browser = await chromium.launch({
  ...
  ...(process.env.BUILDER_EVAL_BROWSER
    ? { executablePath: process.env.BUILDER_EVAL_BROWSER }
    : {}),
})

const results: Array<BuilderAiEvalAttemptResult> = []
try {
...
console.log(`\n${passed}/${results.length} builder eval runs passed.`)
console.log(`Results: ${path.join(options.outputDirectory, 'results.json')}`)
process.exitCode = passed === results.length ? 0 : 1

async function runCase(testCase: BuilderAiEvalCase, run: number) {
  ...
  let checks: Array<BuilderAiEvalCheckResult> = []
  let passed = false

  try {
    try {
      await page.goto(`${options.baseUrl}/builder/ai`, {
        waitUntil: 'domcontentloaded',
        timeout: 60_000,
      })
      const editor = page.locator('section[aria-label="Builder AI editor"]')
      await editor.waitFor({ state: 'visible', timeout: 60_000 })
      await page.waitForFunction(
        () => {
          const bridge: unknown = Reflect.get(
            window,
            '__TANSTACK_BUILDER_AI_EVAL__',
          )
      ...
      selectedModel = await page
        .locator('[data-builder-ai-selected-model]')
        .getAttribute('data-builder-ai-selected-model')

      await page.getByLabel('Builder change').fill(testCase.prompt)
      await page.getByRole('button', { name: 'Send message' }).click()
      await page.waitForFunction(
        () =>
          document
            .querySelector('section[aria-label="Builder AI editor"]')
      ...
      const execution = parseBuilderAiExecution(executionValue)
      await fs.writeFile(
        path.join(artifactDirectory, 'execution.json'),
        `${JSON.stringify(execution, null, 2)}\n`,
      )

      checks = gradeBuilderAiWorkspace(execution, testCase.workspaceChecks)
  ...
  return {
    ...
    checks,
    artifactDirectory,
  } satisfies BuilderAiEvalAttemptResult
}
...
async function configureConnection(
  ...
  if (connection === 'chatgpt') {
    try {
      await page.getByLabel('Builder change').waitFor({
        state: 'visible',
        timeout: 30_000,
      })
    } catch {
      throw new Error(
        'ChatGPT is not connected. Open /builder/ai, choose Continue with ChatGPT, and finish device login first.',
      )
    }
  ...
  const prompt = page.getByLabel('Builder change')
...
async function selectModel(
  ...
  const item = page.locator(
    `[data-builder-ai-connection="${connection}"][data-builder-ai-model="${model}"]`,
  )
...
async function gradePreviewSteps(
  page: Page,
  steps: ReadonlyArray<BuilderAiEvalPreviewStep>,
  caseTimeoutMs: number,
) {
  const results: Array<BuilderAiEvalCheckResult> = []
  for (const step of steps) {
    ...
    if (step.documentMustPersist) {
      ...
      await frame.evaluate((token) => {
        Reflect.set(globalThis, '__tanstackBuilderEvalDocument', token)
      }, documentToken)
    }
    ...
    if (documentToken) {
      ...
      const currentToken: unknown = await frame.evaluate(() =>
        Reflect.get(globalThis, '__tanstackBuilderEvalDocument'),
      )
      results.push({
...
async function pollPreviewCheck(
  page: Page,
  check: BuilderAiEvalPreviewCheck,
  timeoutMs: number,
) {
  const deadline = Date.now() + timeoutMs
  let lastResult: BuilderAiEvalCheckResult | undefined
  do {
    try {
...
async function gradePreviewCheck(
  frame: Frame,
  check: BuilderAiEvalPreviewCheck,
): Promise<BuilderAiEvalCheckResult> {
  switch (check.kind) {
    case 'selector': {
...
async function getPreviewFrame(page: Page, timeoutMs: number) {
  const iframe = page.locator('iframe[title="AI builder spike output"]')
  ...
  if (!frame) throw new Error('Builder preview frame is unavailable')
  return frame
}
...
function selectCases(caseIds: ReadonlyArray<string>) {
  if (!caseIds.length) return builderAiEvalCases
  const selected = builderAiEvalCases.filter((testCase) =>
    caseIds.includes(testCase.id),
  )
  const missing = caseIds.filter(
    (id) => !builderAiEvalCases.some((testCase) => testCase.id === id),
  )
  if (missing.length)
...
function parseOptions(args: ReadonlyArray<string>): Options {
  let baseUrl = process.env.BUILDER_EVAL_BASE_URL ?? 'http://localhost:4173'
...
async function assertServerAvailable(baseUrl: string) {
  try {
    const response = await fetch(`${baseUrl}/builder/ai`)
    if (!response.ok) throw new Error(`HTTP ${response.status}`)
  } catch (error) {
    throw new Error(
      `Builder dev server is unavailable at ${baseUrl}: ${formatError(error)}`,
    )
  }
}

async function writeSummary(
  results: ReadonlyArray<BuilderAiEvalAttemptResult>,
) {
  const passed = results.filter((result) => result.passed).length
  await fs.writeFile(
...
function printResult(result: BuilderAiEvalAttemptResult) {
  console.log(
