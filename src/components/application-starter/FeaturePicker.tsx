 ...
 * Feature Picker (v2)
 *
 * Component for selecting features in the application starter.
...
import type { FeatureId, FeatureInfo } from '~/application-starter/api'
...
const applicationStarterFeaturePlacementContext = getPartnerPlacementContext({
  orderStrategy: 'contextual-recommendation',
  surface: 'application_starter_feature_picker',
})
...
function getPartnerPlacementRankById(features: Array<FeatureInfo>) {
  ...
  return new Map(
    getPartnersForPlacement(
      featurePartners,
      applicationStarterFeaturePlacementContext,
...
function FeatureCard({ feature, categoryColor }: FeatureCardProps) {
  const toggleFeature = useApplicationStarterStore((s) => s.toggleFeature)
  const getFeatureInfo = useApplicationStarterStore((s) => s.getFeatureInfo)
...
export function FeatureOptions({ featureId }: FeatureOptionsProps) {
  const feature = useApplicationStarterStore((s) => s.getFeatureInfo(featureId))
...
function ExamplePicker() {
  ...
  const selectExample = useApplicationStarterStore((s) => s.selectExample)
