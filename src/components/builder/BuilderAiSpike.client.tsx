...
import { builderStarterSource } from '~/utils/builder-environment'
import {
...
const initialDefinition = {
  ...
  workspace: createExampleWorkspace({
    entry: '/index.tsx',
    environment: 'client',
    files: { '/index.tsx': builderStarterSource },
  }),
...
export function BuilderAiSpike() {
  ...
  React.useEffect(() => {
    ...
    window.__TANSTACK_BUILDER_AI_EVAL__ = bridge
    return () => {
      if (window.__TANSTACK_BUILDER_AI_EVAL__ === bridge) {
        delete window.__TANSTACK_BUILDER_AI_EVAL__
      }
    }
  }, [])
  ...
  function applyAiExecution(
    ...
    const hiddenFiles = getBuilderAiHiddenFiles(
  ...
  async function restoreAiExecution(execution: BuilderAiExecution) {
    ...
    <main className="fixed inset-x-0 top-[var(--navbar-height)] bottom-0 z-20 flex min-h-0 flex-col overflow-hidden bg-background-default text-text-primary">
      <header className="flex h-16 shrink-0 items-center gap-3 border-b border-border-subtle bg-background-default px-3 sm:px-4">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-sm font-semibold">AI builder spike</h1>
