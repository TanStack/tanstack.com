...
import { BuilderGuideDialog } from '~/components/charts/BuilderGuideDialog'
...
const draftStorageKey = 'tanstack-charts-builder-source:v1'
const defaultBuilderTitle = 'Builder'
...
const builderImportMap = JSON.stringify({ imports: builderImports })
let esbuildInitialization: Promise<void> | undefined

async function compileBuilderSource(source: string) {
...
<html class="${theme}" style="color-scheme: ${theme}">
  <head>
    ...
    <script type="importmap">${builderImportMap}</script>
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      :root {
        ...
        --builder-foreground: #171717;
        --builder-error: #b91c1c;
      }
      :root.dark {
        color-scheme: dark;
        --builder-background: #030712;
        --builder-foreground: #f9fafb;
        --builder-error: #f87171;
      }
      body {
        overflow-x: auto;
        overflow-y: hidden;
      ...
      #output > pre[data-error] { color: var(--builder-error); }
    </style>
  </head>
  <body>
    <main id="output" aria-label="Builder output"></main>
    <script type="module">
      ...
      window.addEventListener('message', (event) => {
        ...
        if (
          typeof value !== 'object' ||
          value === null ||
          value.type !== 'tanstack-charts-builder:theme' ||
      ...
      parent.postMessage(
        {
          type: 'tanstack-charts-builder:theme-request',
          runToken,
        },
        '*',
      )
      ...
      function post(status, message) {
        parent.postMessage(
          {
            type: 'tanstack-charts-builder:status',
            runToken,
            status,
            message,
          },
      ...
      function postHeight() {
        ...
        parent.postMessage(
          {
            type: 'tanstack-charts-builder:height',
            runToken,
            height,
          },
          '*',
      ...
      for (const level of ['log', 'info', 'warn', 'error', 'debug']) {
        ...
        console[level] = (...values) => {
          ...
          parent.postMessage(
            {
              type: 'tanstack-charts-builder:console',
              runToken,
              level,
      ...
      try {
        const builderModule = await import(moduleUrl)
        const value =
          typeof builderModule.default === 'function'
            ? await builderModule.default(output)
...
function isSandboxHeightMessage(
  ...
  return (
    ...
    value !== null &&
    'type' in value &&
    value.type === 'tanstack-charts-builder:height' &&
    'runToken' in value &&
...
function readBuilderDescription() {
  return (
...
function isSandboxConsoleMessage(value: unknown): value is {
  ...
  return (
    ...
    value !== null &&
    'type' in value &&
    value.type === 'tanstack-charts-builder:console' &&
    'runToken' in value &&
    typeof value.runToken === 'string' &&
    'level' in value &&
...
function isSandboxMessage(
  ...
  return (
    ...
    value !== null &&
    'type' in value &&
    value.type === 'tanstack-charts-builder:status' &&
    'runToken' in value &&
    typeof value.runToken === 'string' &&
    'status' in value &&
...
function isSandboxThemeRequest(value: unknown): value is { runToken: string } {
  return (
    ...
    value !== null &&
    'type' in value &&
    value.type === 'tanstack-charts-builder:theme-request' &&
    'runToken' in value &&
    typeof value.runToken === 'string'
  )
}

export function ChartsBuilderPage() {
  ...
  const [builderTitle, setBuilderTitle] = React.useState(readBuilderTitle)
  const [builderDescription, setBuilderDescription] = React.useState(
    readBuilderDescription,
  )
  ...
  const syncSandboxTheme = React.useCallback(() => {
    outputFrameRef.current?.contentWindow?.postMessage(
      {
        type: 'tanstack-charts-builder:theme',
  ...
  React.useEffect(() => {
    if (!user || !showBuilder || !sourceReady) return
  ...
  }, [showBuilder, source, sourceReady, user])

  React.useEffect(() => {
    document.title = `${builderTitle.trim() || defaultBuilderTitle} | TanStack`
  }, [builderTitle])
  ...
  function runSource(nextSource: string) {
    ...
    void compileBuilderSource(nextSource)
  ...
  function loadExample(id: string) {
    const example = builderExamples.find((item) => item.id === id)
    if (!example) return

    setSource(example.source)
    updateBuilderTitle(example.title)
    updateBuilderDescription(example.description)
    setShowBuilder(true)
    runSource(example.source)
  }

  function createBuilder() {
    setSource(builderStarterSource)
    updateBuilderTitle(defaultBuilderTitle)
    updateBuilderDescription('')
    setSourceReady(true)
    setShowBuilder(true)
    setSourceOpen(true)
    runSource(builderStarterSource)
  }
  ...
  function updateBuilderTitle(title: string) {
    ...
    if (trimmedTitle && trimmedTitle !== defaultBuilderTitle) {
  ...
  function updateBuilderDescription(description: string) {
  ...
  async function share() {
    try {
      ...
      const trimmedDescription = builderDescription.trim()
        ...
        <section className="mx-auto w-full max-w-3xl">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button type="button" size="sm" onClick={createBuilder}>
          ...
          >
            <div className="min-w-0 flex-1">
              <input
                ...
                value={builderTitle}
                maxLength={120}
                onChange={(event) => updateBuilderTitle(event.target.value)}
                onBlur={() => {
              ...
              <input
                ...
                onChange={(event) =>
                  updateBuilderDescription(event.target.value)
                }
                onBlur={() =>
                  updateBuilderDescription(builderDescription.trim())
                }
                      ...
                      >
                        {sourceReady && sandboxDocument ? (
                          <iframe
                            ...
                            title={`${builderTitle.trim() || defaultBuilderTitle} output`}
                            onLoad={() => {
                              syncSandboxTheme()
