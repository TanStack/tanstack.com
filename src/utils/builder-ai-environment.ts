...
export function createBuilderAiEnvironmentSnapshot({
  actualExecution,
  expectedExecution,
  run,
}: {
  actualExecution: BuilderAiExecution
  expectedExecution: BuilderAiExecution
  run: ExampleWorkbenchRunResult
}): BuilderAiEnvironmentSnapshot {
  return {
    expectedRuntime:
      expectedExecution.runtime?.type === 'webcontainer'
        ...
        : 'client',
    executionMatches:
      serializeBuilderAiExecution(expectedExecution) ===
      serializeBuilderAiExecution(actualExecution),
    run: cleanRunResult(run),
  }
}

export function validateBuilderAiCompletion(
  snapshot: BuilderAiEnvironmentSnapshot,
): BuilderAiCompletionDecision {
  ...
  if (snapshot.run.ok && consoleError) {
    return {
      status: 'repair',
      phase: 'runtime',
      diagnostic: consoleError.text || 'The builder logged an error.',
    }
  }
...
export function shouldRestoreBuilderAiCheckpoint({
  currentExecution,
  lastStagedExecution,
}: {
  currentExecution: BuilderAiExecution
  lastStagedExecution: BuilderAiExecution
}) {
  return (
    serializeBuilderAiExecution(currentExecution) ===
    serializeBuilderAiExecution(lastStagedExecution)
  )
}

export function formatBuilderAiEnvironmentEvidence(
  snapshot: BuilderAiEnvironmentSnapshot,
) {
