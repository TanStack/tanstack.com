...
import { validateApplicationStarterGetRequest } from "~/application-starter/api/request-boundary.server";
import { jsonResponse } from "~/utils/api-boundary.server";
import { RATE_LIMITS } from "~/utils/rateLimit.server";

export const Route = createFileRoute("/api/application-starter/deploy/check-name")({
  server: {
    handlers: {
