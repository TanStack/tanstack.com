...
export function ProjectConfig({ hideProjectName }: ProjectConfigProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Router Mode - hidden when not editable (forced to file-router) */}
      {modeEditable && (
        <div>
      ...
      {/* Feature Toggles - only show if at least one is editable */}
      {(typescriptEditable || tailwindEditable) && (
...
function FeatureToggle({ label, enabled, onChange, icon }: FeatureToggleProps) {
  return (
    <button
      type="button"
