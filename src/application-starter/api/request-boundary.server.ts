...
export type ApplicationStarterJsonRequestResult<T> =
  | { body: T; rateLimit: RateLimitResult }
  | { response: Response }

export async function readApplicationStarterJsonRequest(
  request: Request,
  options: { rateLimit?: RateLimitOptions } = {},
): Promise<ApplicationStarterJsonRequestResult<unknown>> {
  const rateLimit = await checkIpRateLimit(
    request,
    options.rateLimit ?? RATE_LIMITS.applicationStarterCompile,
  )
...
export async function validateApplicationStarterGetRequest(
  request: Request,
  options: { rateLimit?: RateLimitOptions } = {},
) {
  const rateLimit = await checkIpRateLimit(
...
export function applicationStarterJsonResponse(body: unknown, rateLimit?: RateLimitResult) {
  return jsonResponse(body, { headers: rateLimit?.headers })
}

export function applicationStarterErrorResponse(
  message: string,
  status: number,
...
export function applicationStarterInternalErrorResponse(message: string) {
