...
} from '../scripts/builder-ai-validation-broker'
import { createExampleWorkspace } from '../src/utils/example-workspace'
import type { BuilderAiLocalValidationRequest } from '../src/utils/builder-ai-local-validation'
...
test('validation broker resolves one correlated result and rejects a duplicate', async () => {
  const broker = new BuilderAiValidationBroker(1_000)
  const requests: Array<BuilderAiLocalValidationRequest> = []
...
test('validation broker removes a timed out request', async () => {
  const broker = new BuilderAiValidationBroker(5)
  const requests: Array<BuilderAiLocalValidationRequest> = []
  const pending = broker.waitForResult(
    createState(),
    new AbortController().signal,
