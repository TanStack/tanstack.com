...
// Token prefixes
const ACCESS_TOKEN_PREFIX = 'mcp_'
const REFRESH_TOKEN_PREFIX = 'mcpr_'
...
export async function createAuthorizationCode(params: {
  ...
  await db.insert(oauthMcpAuthorizationCodes).values({
    codeHash,
...
> {
  ...
  const authCodes = await db
    .select()
    .from(oauthMcpAuthorizationCodes)
    .where(eq(oauthMcpAuthorizationCodes.codeHash, codeHash))
    .limit(1)
  ...
  if (authCode.expiresAt < new Date()) {
    await db
      .delete(oauthMcpAuthorizationCodes)
      .where(eq(oauthMcpAuthorizationCodes.id, authCode.id))
    return { success: false, error: 'invalid_grant' }
  }
  ...
  const accessTokenResult = await db
    ...
    .returning({ id: oauthMcpAccessTokens.id })

  // Insert refresh token
  await db.insert(oauthMcpRefreshTokens).values({
...
> {
  ...
  const refreshTokens = await db
    ...
    .where(eq(oauthMcpRefreshTokens.tokenHash, tokenHash))
    .limit(1)
  ...
  if (token.expiresAt < new Date()) {
    await db
      .delete(oauthMcpRefreshTokens)
      .where(eq(oauthMcpRefreshTokens.id, token.id))
    return { success: false, error: 'invalid_grant' }
  }
...
/**
 * Validate an OAuth access token for MCP requests
 */
export async function validateOAuthToken(token: string): Promise<AuthResult> {
  const tokenHash = await hashToken(token)

  const result = await db
    .select({
      id: oauthMcpAccessTokens.id,
      userId: oauthMcpAccessTokens.userId,
      clientId: oauthMcpAccessTokens.clientId,
      expiresAt: oauthMcpAccessTokens.expiresAt,
    })
    .from(oauthMcpAccessTokens)
    .where(eq(oauthMcpAccessTokens.tokenHash, tokenHash))
    .limit(1)
  ...
  db.update(oauthMcpAccessTokens)
    .set({ lastUsedAt: new Date() })
    .where(eq(oauthMcpAccessTokens.id, accessToken.id))
    .catch(() => {})

  return {
    success: true,
    keyId: accessToken.id, // Use token ID as the "key" ID for rate limiting
...
export async function revokeTokensForClient(
  ...
  await db
    ...
    .where(
      and(
        eq(oauthMcpRefreshTokens.userId, userId),
        eq(oauthMcpRefreshTokens.clientId, clientId),
      ),
    )

  // Delete access tokens
  await db
    .delete(oauthMcpAccessTokens)
    .where(
      and(
        eq(oauthMcpAccessTokens.userId, userId),
        eq(oauthMcpAccessTokens.clientId, clientId),
      ),
    )
}
...
export async function cleanupExpiredTokens(): Promise<void> {
  ...
  await db
    .delete(oauthMcpAuthorizationCodes)
    .where(lt(oauthMcpAuthorizationCodes.expiresAt, now))

  // Delete expired refresh tokens
  await db
    .delete(oauthMcpRefreshTokens)
    .where(lt(oauthMcpRefreshTokens.expiresAt, now))

  // Delete expired access tokens
  await db
    .delete(oauthMcpAccessTokens)
    .where(lt(oauthMcpAccessTokens.expiresAt, now))
}
