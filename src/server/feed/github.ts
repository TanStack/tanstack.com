...
async function getLastSyncTime(repo: string): Promise<number | null> {
  ...
  const config = await getFeedConfig({ data: { key: configKey } })
  if (!config) {
    return null
  }
  // config has structure: { _id, key, value, updatedAt }
  // value is JSONB, so we need to cast it
  const configValue = config as { value: any }
  return configValue.value?.lastSyncedAt ?? null
}
      ...
      for (const release of releases) {
        ...
        try {
          ...
          if (existing) {
            ...
            // existing has structure: { _id, id, source, title, content, ..., autoSynced, ... }
            const isAutoSynced = (existing as any).autoSynced ?? false
            await updateFeedEntry({
              data: {
                ...
                publishedAt: isAutoSynced ? undefined : normalized.publishedAt,
