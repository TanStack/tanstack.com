...
async function requireCapability({
  ...
  const hasAccess =
    ...
    user.capabilities.includes(capability as Capability)
  if (!hasAccess) {
...
export async function listUsers({ data }: { data: any }) {
  ...
  if (
    ...
    conditions.push(
      arrayOverlaps(users.capabilities, data.capabilityFilter as Capability[]),
    )
  }
  ...
  if (
    ...
    filteredUsers = allMatchingUsers.filter((user) => {
      ...
      if (data.capabilityFilter && data.capabilityFilter.length > 0) {
        ...
        return data.capabilityFilter.some((cap: Capability) =>
