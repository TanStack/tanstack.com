...
excerpt: TanStack Table V9 can use dramatically less memory than V8 in large tables because row, column, cell, and header APIs now live on shared prototypes instead of every object instance.
library: table
authors:
  - Kevin Van Cott
---
...
Up to 90% less memory usage in TanStack Table V9 compared to V8? Yes! For large tables at least.
...
But one of the largest performance wins during the development of V9 came from a more subtle behind-the-scenes refactor. This refactor resulted in TanStack Table V9 using **up to ~90% less memory** than V8 for large tables when needing to process hundreds of thousands or millions of rows, either paginated or virtualized.

This improvement is either a big deal or one that barely matters, depending on how you want to use TanStack Table. In V8, depending on a large number of factors, you could only expect TanStack Table to be able to handle about 1 million (1.5 million at most) rows before running into memory issues in the browser (which is often the 4GB mark). With V9, the maximum number of rows that TanStack Table can handle before using 4GB of memory is now 10-16 million rows according to our benchmarks. Though note, that depending on how complex your web page is, you will most likely run into issues below these optimistic numbers, but this is up to a 10x improvement in the scalability of TanStack Table itself.
...
![chart showing the memory usage of TanStack Table V8 and V9 across paginated rows, virtualized rows, virtualized columns, and kitchen sink examples](/blog-assets/tanstack-table-v9-memory-performance/charts.png)

The charts above show that as the number of cells (rows x columns) that TanStack Table is asked to process (not just render) grows, the memory usage differences between V8 and V9 become much more pronounced. On the left of the graph, the difference is negligible, but on the right, V9 uses more than **2.4 GB less retained JS heap** than V8 when processing 1 million rows x 8 columns.

Here are the full benchmark results:

| Benchmark Example   | # of Cells (rows x columns) | V8 Memory Used | V9 Memory Used | Memory Less | Percentage Improvement |
| ------------------- | --------------------------: | -------------: | -------------: | ----------: | ---------------------: |
...
### Why Not Do This in V8?

There was an original [PR](https://github.com/TanStack/table/pull/5927) from [Michael Leibman](https://github.com/mleibman-db) that proposed this kind of refactor for TanStack Table V8. However, we discovered that this technically introduces subtle breaking changes. So we decided that this would be less risky to implement in V9 instead.
...
This worked in V8 because methods like `getValue` were arrow functions created inside the row factory. They closed over the `row` object, so they did not care how they were called.

In V9, the method is shared on the row prototype and uses its `this` context to know which row it is operating on. When you destructure a method like this, you get the function, but you lose the original receiver. Then `this` is `undefined` in strict mode, so the method cannot find the row.

This is also why the methods no longer show up as own properties on the `row` object. They are on the prototype. If you console.log or console.dir the `row` object, you will see that the methods are not there.
...
This is a tradeoff worth making in a breaking change release, especially if it is the only downside, but we couldn't push this change out into V8.
