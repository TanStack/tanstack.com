...
import { shouldAutoRunBuilder } from '../src/utils/builder-auto-run.client'
...
for (const [label, platform, maxTouchPoints, userAgent, expected] of cases) {
  test(`builder auto-run policy handles ${label}`, () => {
    assert.equal(
      shouldAutoRunBuilder({ maxTouchPoints, platform, userAgent }),
      expected,
    )
  })
}
