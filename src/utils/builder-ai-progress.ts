...
export type BuilderAiRepairContext = {
  priorEvidenceFingerprints: Array<string>
  blockedMutationFingerprints: Array<string>
}

export type BuilderAiAttemptTrace = {
  evidenceFingerprints: Array<string>
  mutationFingerprints: Array<string>
}

export type BuilderAiFailureObservation = BuilderAiAttemptTrace & {
  failureFingerprint: string
  executionFingerprint: string
}

export type BuilderAiFailureProgress = {
  history: Array<BuilderAiFailureObservation>
  repair: BuilderAiRepairContext
  repeatedState: boolean
}

export type BuilderAiProgressGate = {
...
export function createBuilderAiProgressGate(
  repair: BuilderAiRepairContext | undefined,
): BuilderAiProgressGate {
  ...
  return {
    ...
    recordEvidence(toolName, input, output) {
      ...
      evidenceFingerprints.add(
        fingerprintBuilderAiValue({ toolName, input, output }),
      )
    },
    assertCanMutate(toolName, input) {
      ...
      if (required && !hasFreshEvidence) {
        throw new Error(
          'Gather new evidence that differs from prior repair attempts before mutating the builder.',
        )
      }

      const fingerprint = fingerprintBuilderAiValue({ toolName, input })
      if (
...
export function parseBuilderAiRepairContext(
  value: unknown,
): BuilderAiRepairContext | undefined {
  ...
  if (
    ...
    throw new Error('Invalid builder AI repair context')
  }
...
export function parseBuilderAiAttemptTrace(
  value: unknown,
): BuilderAiAttemptTrace {
  if (
    !isRecord(value) ||
    ...
    throw new Error('Builder AI returned an invalid attempt trace')
  }
...
export function fingerprintBuilderAiValue(value: unknown) {
...
export function fingerprintBuilderAiDiagnostic(phase: string, message: string) {
  return fingerprintBuilderAiValue({
    phase,
    message: message.trim().replace(/\s+/g, ' '),
  })
}

export function recordBuilderAiFailure(
  history: ReadonlyArray<BuilderAiFailureObservation>,
  observation: BuilderAiFailureObservation,
): BuilderAiFailureProgress {
