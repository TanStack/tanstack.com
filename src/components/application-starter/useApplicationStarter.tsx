...
import {
  trackEvent,
  defaultApplicationStarterSessionContext,
...
interface UseApplicationStarterOptions {
...
export function useApplicationStarter({
  ...
  // Session context (mode_used, idea_used) is stamped on every applicationStarter
  ...
  const sessionContextRef = React.useRef<ApplicationStarterSessionContext>(
    defaultApplicationStarterSessionContext,
  )
  ...
  const handleCopy = React.useCallback(
    async (
      ...
      notify(
        ...
        { id: `applicationStarter-copied-${kind}` },
      )
    },
    [markCopied, notify],
  )
  ...
  const resolveSubmittedInput = React.useCallback(
    async (
      ...
      options?: {
        applyApplicationStarter?: boolean
        silentApplicationStarter?: boolean
      },
    ) => {
      ...
      try {
        ...
        if (options?.applyApplicationStarter && applicationStarterIntegration) {
          await applicationStarterIntegration.applyResult(nextResult, {
            silent: options.silentApplicationStarter,
          })
        }
        ...
        if (requestId === latestRequestIdRef.current) {
          ...
          trackEvent('application_starter_failed', {
            ...sessionContextRef.current,
            stage: 'generation',
            error_message:
    ...
    [applyResolvedResultState, applicationStarterIntegration, context, notify],
  )
  ...
  const selectSuggestion = React.useCallback(
    async ({
      ...
      // Stamp the idea on subsequent applicationStarter events so any downstream
  ...
  const trackAction = React.useCallback(
    (
      action: ApplicationStarterAction,
      provider?: StarterDeployProvider | null,
    ) => {
      trackActivation({
        action,
        surface: 'result_panel',
  ...
  const navigateToResult = React.useCallback(
    async (kind: 'advanced' | 'download') => {
      await withResolvedResult((nextResult) => {
        ...
        if (
          kind === 'download' &&
          applicationStarterIntegration?.downloadResult
        ) {
          return applicationStarterIntegration.downloadResult(nextResult)
        }

        const destination =
          kind === 'download'
            ? nextResult.downloadUrl
            : nextResult.advancedApplicationStarterUrl
    ...
    [applicationStarterIntegration, trackAction, withResolvedResult],
  )
  ...
  const generatePrompt = React.useCallback(async () => {
    ...
    if (applicationStarterIntegration) {
      const applied = await applicationStarterIntegration.applyResult(
        nextResult,
        {
          silent: false,
        },
      ...
      if (applied) {
        notify(
          <div>
            <div className="font-medium">ApplicationStarter configured</div>
            <div className="text-xs text-gray-500 dark:text-gray-400">
              The prompt was applied to the application starter immediately.
            </div>
          </div>,
        )
      }
  ...
  const resetApplicationStarter = React.useCallback(() => {
    latestRequestIdRef.current += 1
