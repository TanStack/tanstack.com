            ...
            >
              {mountedRows.map((index) => (
                ...
                >
                  ...
                  />
                  <span className="w-16 shrink-0 font-ds-mono text-[9px] text-white/25">
    ...
    <LandingWindow label="chat viewport">
      <div className="p-4 sm:p-5">
        ...
        <div className="relative mt-4 h-60 overflow-hidden rounded-lg border border-white/6 bg-black/30 p-4">
          ...
          {unseenCount > 0 ? (
            <button
              className="absolute right-3 bottom-3 rounded-full bg-[var(--landing-accent)] px-3 py-1.5 font-ds-mono text-[9px] text-[var(--landing-accent-ink)]"
              onClick={() => {
                setAtLatest(true)
                setUnseenCount(0)
              }}
