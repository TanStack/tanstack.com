        ...
        bridgeReady = new Promise<number>((resolve, reject) => {
          const child = spawn(
            ...
            [
              '--import',
              'tsx',
              path.join(server.config.root, 'scripts/builder-ai-app-server.ts'),
            ],
            {
          ...
          const timeout = setTimeout(() => {
            child.kill()
            reject(new Error('Builder AI bridge timed out while starting'))
          }, 30_000)
          ...
          child.once('exit', (code) => {
            ...
            bridgeProcess = undefined
            bridgeReady = undefined
            reject(new Error(`Builder AI bridge exited (${code ?? 'unknown'})`))
          })
        })
        return bridgeReady
      }
...
async function pipeBridgeResponse(
  ...
  try {
    while (true) {
      ...
      if (size > limit) {
        await reader.cancel()
        throw new ProxyError(502, 'Builder AI bridge response too large')
      }
