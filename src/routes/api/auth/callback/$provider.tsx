  ...
  server: {
    handlers: {
      GET: async ({
        ...
        const [authIndex, audit, activity] = await Promise.all([
          import("~/auth/index.server"),
          import("~/utils/audit.server"),
          import("~/utils/activity.server"),
        ]);
        const {
          getOAuthStateCookie,
        ...
        const { recordLogin } = audit;
        const { recordDailyActivity } = activity;
        ...
        try {
          ...
          Promise.all([
            ...
            console.error(
              "[AUTH:WARN] Failed to record login/activity event:",
              err,
            );
          });
