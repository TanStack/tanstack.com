...
import { useApplicationStarterStore, useCustomTemplate } from './store'
import type { CustomTemplateCompiled } from '~/application-starter/api'
...
export function CustomTemplateDialog({ onClose }: CustomTemplateDialogProps) {
  ...
  const setCustomTemplate = useApplicationStarterStore(
    (s) => s.setCustomTemplate,
  )
  ...
  const handleLoad = async () => {
    ...
    try {
      const response = await fetch(
        `/api/application-starter/load-remote-template?url=${encodeURIComponent(url)}`,
      )
    ...
    <div className="space-y-4">
      ...
      {error && (
        <div className="flex items-start gap-2 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-sm text-red-700 dark:text-red-400">
          <WarningCircleIcon className="w-4 h-4 mt-0.5 shrink-0" />
          <span>{error}</span>
        </div>
      )}
